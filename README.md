...
| 模块 | 变更 |
|---|---|
| L2 子模块折叠 + 状态摘要 | timeline 每个扩产计划下的 4 个 L2 模块（阀点 / 审批 / 试产 / 爬坡）支持独立折叠；折叠时在标题旁显示该计划的完成态摘要 chip（已完成 / 进行中 / 未开始 计数 + 已逾期红框粗体警示），展开时 chip 隐藏避免干扰；折叠状态按 planId 写入 `localStorage`，刷新页面后保留 |
| 阀点关键交付物 + 悬浮说明 | MILESTONE_TEMPLATE 增加 `deliverables` 字段，列出 8 个标准阀点的关键交付物清单（可研报告 / 投资批复 / 基础设计 / 政府审批许可与核心设备 / 土建与安装验收 / 试产考核报告 / 客户认证 / 爬坡计划 + OEE）；阀点卡片「佐证」字段 hover 时弹出浅色气泡展示该阀点的交付物清单 |
| 阀点 / 试车 命名统一 | 阀点模板第 4 项「政府审批与许可」→「关键审批与设备」、第 6 项「试车验证与考核」→「试产验证与考核」；L2 模块「试车验证记录」→「试产验证记录」；验证项目「单机 / 联动 / 投料 试车」→「单机 / 联动 / 投料 试产」；风险类型「试车不达标」→「试产不达标」（前后端、UI 文案同步更新） |
| L2 折叠摘要 chip 优先级 | `summarizeMilestones` 等函数判断顺序调整为「已逾期 → 已完成 → 进行中 → 未开始」，保证 status='已逾期' + actualArrival 已存在的「逾期补录」节点不会被误算成已完成；逾期 chip 加 ⚠ + 红框粗体，与普通计数 chip 视觉区分 |
| 风险模型升级（9 类 + 4 档） | `risk` 表新增 `sourceKind / sourceId / sourcePlanId` 反向链接字段；风险类型从 5 类扩展到 9 类（5 物料级 + 4 L2 节点级：审批逾期 / 试产不达标 / 爬坡未达标 / 阀点延期）；后端 `db.py` 加幂等迁移，列不存在自动 ALTER。等级 enum 仍是 4 档（RED/ORANGE/YELLOW/GREEN），UI 只暴露 3 档（ORANGE 与 YELLOW 同显为黄）。 |
...
| Timeline pendingRiskSignal | timeline 4 个 L2 模块（阀点/审批/试产/爬坡）每个节点都加 `pendingRiskSignal` 字段，按规则自动判级：阀点延期 >7 天 YELLOW / >30 天 ORANGE；审批逾期 >14 天 ORANGE / >30 天 RED；试产 FAIL → RED；爬坡 < 目标 80% → ORANGE |
...
| 扩产计划 CRUD | 时间轴页面支持「新增 / 归档」扩产计划；新建可一键按模板自动生成 8 阀点 + 6 审批 + 6 试产 + 4 爬坡共 24 个子节点；删除改为软删除（打 `archived_at` 时间戳，列表自动过滤，数据不丢） |
| 编辑计划 · 子节点计划时间 | 新增「子节点计划时间」编辑面板，4 个 L2 模块（阀点 / 审批 / 试产 / 爬坡）共 24 个计划日期可一次性手动微调，底部「保存全部」按钮按 dirty 标记批量 PATCH |
...
| 证据预览 | 关键审批 / 试产验证 / 量产爬坡 / 证据档案四处的佐证 chip 改为可点击，直接内嵌预览图片 / PDF |
...
|---|---|---|---|
| **全部阀点明细** | `expansion_item`（每计划 8 张卡片） | 阀点序号 / 名称 / 状态 / 计划&实际日期 / 供应商&采购侧 | 一张卡片看清一个阀点的双方责任与时间偏差；hover「佐证」字段弹出该阀点的关键交付物清单 |
| **关键审批事项进度** | `approval`（6 项标准审批） | 审批类型 / 审批机构 / 提交/预计批复/实际批复日期 / 状态 / 备注 | 监控政府批文进度，悬浮查看准备周期（环评 6-18 个月等） |
| **试产验证记录** | `commissioning_item`（6 项验证） | 目标值 / 实测值 / 合格判定 / 验证日期 / 备注 | 设备调试到量产前的质量关卡，留痕可追溯 |
| **量产爬坡计划跟踪** | `ramp_item`（4 阶段爬坡） | 阶段 / 负荷率 / 目标产能 / 计划周期 / 实际达成 / 达标状态 | 监控从 40% 负荷到 100% 全产能的爬坡节奏 |

每个 L2 模块支持独立折叠（chevron 按钮 + localStorage 记忆）；折叠时标题旁显示该计划的完成态摘要 chip（如「6/8 已完成 · 1 进行中 · 1 未开始」「⚠ 已逾期 3 项」），展开时 chip 隐藏。已逾期 / 未通过 chip 用 ⚠ + 红框粗体作为首要警示，与普通计数 chip 视觉区分。
...
| `/board/expansion/view/timeline` | 里程碑时间轴（8 阀点 + 政府批文 + 试产 + 爬坡） |
...
- **CommissioningItem**（试产验证项）：计划 id、验证类型、目标值、实测值、合格判定（PASS/FAIL/IN_PROGRESS/PENDING）、验证日期、备注
...
| `COMMISSIONING_FAIL` | 试产不达标 | 试产 `passStatus == FAIL` |
...
|---|---|---|---|
| `RED` | 红 | red | 试产 FAIL · 审批逾期 >30 天 · 关闭前最高告警 |
...
| # | Key | 名称 | 关键交付物（hover 佐证可见） |
|---|---|---|---|
| 1 | `FEASIBILITY` | 需求确认与可行性研究 | 可研报告 / 市场预测 |
| 2 | `EIA` | 项目立项与审批 | 投资批复 / 立项文件 / 双方盖章版《产能承诺书》 |
| 3 | `EQUIPMENT_ORDER` | 工艺设计与工程 | 基础设计 / 详细设计 |
| 4 | `CIVIL` | 关键审批与设备 | 政府审批许可：环评批复、安评批复、排污许可、危化品生产许可、建设用地规划许可、节能审查；核心设备：全套采购订单、到货确认单、设备合格证 |
| 5 | `EQUIPMENT_DELIVERY` | 施工建设与安装 | 厂房 / 仓储 / 环保配套土建竣工验收单 + 现场照片；全套生产设备安装验收单 |
| 6 | `INSTALLATION` | 试产验证与考核 | 试产报告 / 72h 满负荷考核报告 |
| 7 | `TRIAL_PRODUCTION` | 客户认证与审核 | 样品检测报告 / CATL 变更现场审核通过报告 |
| 8 | `FULL_PRODUCTION` | 量产爬坡与优化 | 爬坡计划 / OEE 数据 |
...
| # | Key | 名称 | 验证标准 |
|---|---|---|---|
| 1 | `SINGLE_TRIAL` | 单机试产 | 设备空载运行2h无异常 |
| 2 | `INTEGRATED_TRIAL` | 联动试产 | 全流程联动运行8h无异常 |
| 3 | `FEED_TRIAL` | 投料试产 | 按配方投料，产出合格产品 |
...
A: 扩产管理的核心闭环是：里程碑节点 → 政府批文 → 试产验证 → 量产爬坡。4 个模块串起来覆盖从立项到达产的全过程。